...
export default function App() {
  ...
  const [enrollResult, setEnrollResult] = useState(null); // { biometric_reference_id, enrolled: true }
  ...
  const fileToBase64 = (file) => new Promise((resolve, reject) => {
    ...
    const handleEnrollBiometric = async () => {
      ...
      try {
        ...
        const referenceId = claimsByType[selectedType]?.biometric_reference_id || claimsByType.BiometricCredential?.biometric_reference_id;
          ...
          <section className="card">
            ...
            {issueMode === 'single' ? (
              ...
              <form onSubmit={handleIssueBatch} className="form">
                ...
                {(selectedType === 'BiometricCredential' || selectedType === 'ConsolidatedCredential') && (
                  <div className="field-group" style={{ borderTop: '1px solid #f3f4f6', paddingTop: '12px' }}>
                    ...
                    {biometricEnabled && (
                      <>
                        ...
                        <button type="button" onClick={handleEnrollBiometric} disabled={!selfieFile || isEnrolling} className="button" style={{ marginTop: '8px' }}>
                          {isEnrolling ? 'Enrolling...' : 'Enroll Biometric'}
                        </button>
                        {enrollResult && <p style={{ color: '#059669' }}>✓ Enrolled successfully</p>}
                        {enrollError && <div className="error-box">{enrollError}</div>}
                      </>
                    )}
                  </div>
                )}
